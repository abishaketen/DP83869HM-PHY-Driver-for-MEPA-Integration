...
#ifndef _MEPA_TI_PRIVATE_H_
#define _MEPA_TI_PRIVATE_H_
...
/*#define TI_CABLE_MODE_POWER_DOWN 2

#define TI_CABLE_NORMAL 0
#define TI_CABLE_OPEN   1
#define TI_CABLE_SHORT  2
#define TI_CABLE_FAIL   3*/
...
//mepa_rc ti_ext_reg_rd(mepa_device_t *dev, uint16_t page, uint16_t addr, uint16_t *value);
//mepa_rc ti_ext_reg_wr(mepa_device_t *dev, uint16_t page, uint16_t addr, uint16_t value, uint16_t mask);
...
/*mepa_rc ti_ts_debug_info_dump(struct mepa_device *dev,
                                const mepa_debug_print_t pr,
                                const mepa_debug_info_t   *const info);*/

//Direct register access macros
#define RD(dev, addr, value) ti_direct_reg_rd(dev, addr, value)
#define WR(dev, addr, value) ti_direct_reg_wr(dev, addr, value, 0xffff)
#define WRM(dev, addr, val, mask) ti_direct_reg_wr(dev, addr, val, mask)

//Extended page register access macros
//#define EP_RD(dev, page_addr, value) ti_ext_reg_rd(dev, page_addr, value)
//#define EP_WR(dev, page_addr, value) ti_ext_reg_wr(dev, page_addr, value, 0xffff)
//#define EP_WRM(dev, page_addr, value, mask) ti_ext_reg_wr(dev, page_addr, value, mask)

//MMD device register access macros
#define MMD_RD(dev, mmd_addr, value) ti_mmd_reg_rd(dev, mmd_addr, value , 0xffff)
#define MMD_WR(dev, mmd_addr, value) ti_mmd_reg_wr(dev, mmd_addr, value, 0xffff)
#define MMD_WRM(dev, mmd_addr, val, mask) ti_mmd_reg_wr(dev, mmd_addr, val, mask)
...
typedef enum {
    TI_TS_MODE_DISABLED = 0,      // PTP functions are disabled
    TI_TS_MODE_STANDALONE = 1,    // PTP Standalone Mode
    TI_TS_MODE_PCH = 2,           // PTP PCH Mode (MCH support disabled)
    TI_TS_MODE_PCHMCH = 3,        // PTP PCH Mode (MCH support enabled)
} TI_ts_tsu_op_mode_t;
...
typedef struct {
    ...
    TI_ts_port_latencies_t        port_latencies;
} TI_ts_port_conf_t;

typedef struct {
    mepa_bool_t                   ptp_en;               // Chip PTP enabled/disabled
    TI_ts_tsu_op_mode_t         tsu_op_mode;          // TSU operating Mode: Standalone/PCH/PCH-MCH
    ...
    TI_ts_port_conf_t           ts_port_conf;         // Port specific TSU data
    mepa_bool_t                   tx_spi_en;            // TS access through SPI enaled
    mepa_ts_fifo_read_t           fifo_cb;              // Fifo TS callback
    TI_ts_port_latencies_t      default_latencies;    // Default port latencies
